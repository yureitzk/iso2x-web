...
// Maximum time to wait for the browser to pull more data once the
// stream has started applying backpressure. This is a last-resort
// safety net for a stream the browser silently walked away from
// without ever calling cancel(); it's not a liveness check on the
// page, since the heartbeat above already covers that independently
// of pull cadence. The value is generous on purpose: pull() is paced
// by whatever sits downstream of the browser (antivirus/Safe Browsing
// scanning, slow disks), and a real, still-progressing download can
// stall well past a short bound.
...
export function createHandlers(sw) {
	...
	async function handleDownload(id) {
		...
		const headers = /** @type {Record<string, string>} */ ({
			...
			// The page enforces COEP. This response loads in the hidden
			// download iframe, so it needs to declare COEP too or the
			// browser blocks it.
