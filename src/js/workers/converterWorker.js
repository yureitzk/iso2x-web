...
/**
 * Conversion's main read path: a windowed FileReaderSync cache (see
 * windowedFileReader.js). Wraps makeWindowedReadFn() to also feed
 * readTracker.js, which WorkerController.js uses to size WorkerPool.js's
 * recycle decision.
 * @param {File} file
 * @returns {SourceReadFn}
 */
function makeReadFn(file) {
	const windowedRead = makeWindowedReadFn(file, SEQUENTIAL_WINDOW_BYTES);
	return (/** @type {number} */ offset, /** @type {number} */ length) => {
		const bytes = windowedRead(offset, length);
 ...
 * @param {File[]} files
 * @param {(file: File, index: number) => SourceReadFn} makePartReadFn
 * @returns {{
...
function resolveMultiFileSource(files, makePartReadFn) {
	...
	// Multi-file group: already content-verified as a real split - just
	// detect format off part 1 and build SourceParts for the rest.
	const format = detectFormat(readFn0, files[0].size);
	/** @type {SourcePart[]} */
...
async function resolveDroppedSource(source, makePartReadFn) {
	...
	// Not god/extracted-shaped - maybe the folder holds one (possibly
	// split) image instead of loose files. resolveBatchEntry is async: it
	// may need to byte-verify a split pair before committing to it.
	...
	if (resolved.kind === 'invalid') {
		// A folder with a named CCI/CISO pair whose content doesn't verify -
		// surface the real reason through inspectSource()'s catch block.
...
async function inspectSource(source) {
	...
	try {
		 ...
		 * window/page cache: inspection's read pattern has close to zero
		 * locality (routinely 90%+ of reads don't follow the previous
		 * one), so a fixed-size window cache like makeReadFn()'s below
		 * would overfetch heavily without cutting the read count enough
		 * to be worth it.
		 * @param {File} file
		 */
		...
		// titleThumbnail (game icon) wins over thumbnail (content-specific
		// icon, e.g. a save's own icon) - same priority as XBE's
...
async function* streamChunks(
	...
	try {
		yield* pumpSessionChunks(session, waitForRoom, (chunk) => {
			...
			// the chunk - safe to transfer directly instead of copying
			// it again first.
			return {
				value: chunk.buffer,
...
async function* multiFileChunks(
	...
	try {
		yield* pumpSessionChunks(session, waitForRoom, (chunk) => {
			...
			// Same reasoning as streamChunks() above - transfer the
			// buffer directly instead of copying it first.
			return {
				value: {
					name,
					chunk: chunk.buffer,
	...
	switch (format) {
		...
		case 'extracted': {
			...
			return {
				...
				skipSystemUpdate,
				allowedMediaPatch,
				// The checkbox's boolean state, not a string - the title
				// comes from the game-title input.
...
async function convertSource(convert) {
	...
	try {
		// Detects the input file's format once, up front (also verifying a
		// CCI split pair or resolving a god/extracted-shaped folder), and
		// builds per-part readFns as windowed FileReaderSync reads (see
		// makeReadFn() above / windowedFileReader.js).
		const {
			format: sourceFormat,
			readFn,
			fileSize,
			sourceParts,
		} = await resolveDroppedSource(source, (file) => makeReadFn(file));

		// Opened once and reused below (attach-XBE generation, then the
		// conversion session) instead of re-opening the source each step.
		// See OpenedSource's doc comment in iso2x.
		...
		try {
			if (shouldGenerateAttachXbe) {
				...
				// Called before openConversionSession() so a non-OGX source
				// aborts immediately, rather than only failing after the
				// primary output has finished.
				attachXbeBytes = opened.generateAttachXbe();
			}

			// opened.openConversionSession() returns iso2x's raw
			// wasm-bindgen ConversionSession (undefined-based); wrap it so
			// every helper below gets the null-based shape it expects.
			// `opened` is only freed in the catch/no-session branch below,
			// since openConversionSession() consumes it on success.
		...
		if (session === null) {
			// Unreachable in practice (the catch above rethrows on any
			// failure) - only here because TS can't narrow across a
			// try/catch boundary.
			throw new Error(
		...
		// Direct, unzipped, one-download-per-split-file path - used by
		// 'ciso' (always split) and 'xiso' when the split option is on.
		if (isMultiFileDirect) {
			...
			if (!_cancelled) self.postMessage({ type: MSG.DONE });
			return;
		}
...
async function onMessage(e) {
	...
	try {
		...
		if (type === MSG.INSPECT) {
			...
			const { dirName, entries, files } = /** @type {PartitionDirMessage} */ (
				e.data
			);
			let count = 0;
			await partitionDroppedFolder(dirName, entries, files, (source) => {
				count++;
				self.postMessage({ type: MSG.PARTITION_ITEM, payload: source });
			});
			// Payload is just a count - every source already went out
			// individually as PARTITION_ITEM; this just signals batch done.
			self.postMessage({
