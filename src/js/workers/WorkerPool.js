import ConverterWorker from './converterWorker.js?worker';
...
// to pay wasm-init cost recycling them.
const RECYCLE_SIZE_BYTES = 300 * 1024 * 1024; // 300MB
...
export class WorkerPool {
	...
	_parkIdle(worker) {
		...
		const timer = setTimeout(() => {
			...
			if (idx === -1) {
				// Already claimed by acquire() between the timer firing and
				// this callback running - nothing to evict.
				return;
			}
	 ...
	 *
	 * A worker that reads a large File leaves resident memory scoped to
	 * that worker's realm, which nothing but tearing the realm down
	 * reclaims - so a worker is recycled (terminated + replaced) once
	 * `sizeBytes` crosses RECYCLE_SIZE_BYTES, or once it's been used
	 * RECYCLE_AFTER_USES times regardless of size (catches accumulation
	 * across many medium files that never individually cross the size
	 * threshold). Otherwise it's reused, to avoid paying wasm-init cost
	 * on every single probe task.
	 * @param {Worker} worker
	 * @param {number} [sizeBytes] - total bytes of every File this
	 *   release's task just read. Omit/0 for tasks that didn't touch a
	 *   File (shouldn't happen for pool tasks today, but keeps this
	 *   safe to call without it).
	 */
	release(worker, sizeBytes = 0) {
		...
		if (!shouldRecycle) {
			...
			if (waiter) {
				...
				// so it never had an idle timer to clear.
		...
		if (waiter) {
			this._created++;
			// If this eager respawn itself fails, reject the waiter instead
			// of leaving its acquire() promise unsettled forever - see the
			// note on `_waiters` for why both callbacks are needed here.
			// (_spawn()'s own onError already decrements `_created` on
			// failure, so nothing extra to undo here.)
			this._spawn().then(waiter.resolve, waiter.reject);
			return;
		}
		// Don't eagerly respawn idle capacity - let the next acquire()
		// spawn on demand, same as today's cold-start path.
	}
