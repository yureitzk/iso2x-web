 ...
 * @import { SingleDroppedSource, WorkerMessage, NormalizedConvert, PendingPartition, PendingVerify } from '../../types/global'
 */
...
const FORWARDED_EVENTS = new Map([
	...
	[MSG.LOG, EVENTS.LOG],
	// MSG.ERROR is handled explicitly in #onMessage - it must also reclaim the worker.
]);
...
export class WorkerController extends EventTarget {
	...
	async #ensureDownloadSlot() {
		...
		if (this.#downloadSlotState === 'acquiring') {
			...
			// Teardown ran while acquire() was in flight and it still
			// resolved - hand the slot back rather than leak it.
	...
	async #watchForAttach(streamId, filename, totalSize, attempt) {
		...
		// Terminated, or already closed normally via another path - nothing to do.
