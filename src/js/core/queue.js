 ...
 * discs aren't necessarily the same size.
...
export function createSlotQueue(priorityList, maxConcurrent) {
	...
	function drain() {
		if (running >= cap || pending.length === 0) return;
		// Build one position lookup per drain() call instead of calling
		// priorityList.indexOf() per pending waiter, which is O(pending *
		// priorityList.length). Rebuilt from scratch every call, so external
		// reordering of priorityList between calls is still picked up
		// immediately.
		const positionOf = new Map(priorityList.map((e, i) => [e, i]));
		const posFor = (/** @type {QueueEntry} */ e) => positionOf.get(e) ?? -1;

		while (running < cap && pending.length > 0) {
			let bestIdx = 0;
			let bestPos = posFor(pending[0].entry);
			for (let i = 1; i < pending.length; i++) {
				const pos = posFor(pending[i].entry);
				if (pos < bestPos) {
					bestPos = pos;
					bestIdx = i;
				}
