...
import { relativeDirEntries } from '../../lib/helpers.js';
 ...
 */
function swapAdjacent(array, idx, direction) {
...
/**
 * @typedef {object} DiscActionsDeps
 * @property {() => SwBridge} getSwBridge
 * @property {(item: QueueEntry, outcome: SourceOutcome) => void} resolveSource
 * @property {(item: QueueEntry) => void} renderSourceParts
 */
...
export function createDiscActionsController({
	...
	function removeDisc(item, idx) {
		...
		discs.splice(idx, 1);
		// Removing a disc can't change titleId/contentType, so no
		// re-inspect is needed - just the file list/size bookkeeping.
